...
int main(){
    int n;
    cout<<"Enter total number of companies: ";
    cin>>n;
    vector<double>stockPrices(n);
    vector<int>stockRose(n);
    //storing shareprice
    for(int i=0;i<n;i++){
        cout<<"Enter current stock price of company "<<(i+1)<<":";
        cin>>stockPrices[i];
        cout<<"Did the company's stock price rise today compared to yesterday?(1 for true || 0 for false)";
        cin>>stockRose[i];
    }
    cout<<"----------------------------------------------------------------------------------------------------"<<endl;
    int choice;
